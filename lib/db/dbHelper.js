...
module.exports = {
  ...
  getTradingHistory:function(accountNumber,callback){
    ...
    con.query(cmd, function (err,rows,fields){
      ...
      var data ="Your transaction history: \n";
      for (var i = 0 ; i < rows.length; i++){
        if (rows[i].TKNGUOIGUI == accountNumber){
          data+="**************************************************\n";
          data+="send to "+rows[i].TKNGUOINHAN + " "+ rows[i].SOTIEN+" vnd\n";
          data+="your balance was down to "+rows[i].SODUNGUOIGUI+" vnd\n";
          data+="**************************************************\n";
        }
        else {
          data+="**************************************************\n";
          data+="receive from "+rows[i].TKNGUOIGUI + " "+ rows[i].SOTIEN+" vnd\n"; 
          data+="your balance was up to "+rows[i].SODUNGUOINHAN+" vnd\n";
          data+="**************************************************\n";
        }
      }
      callback(true,data);
