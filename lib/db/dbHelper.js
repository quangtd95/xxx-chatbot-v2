  ...
  getTradingHistory:function(accountNumber,callback){
    ...
    con.query(cmd, function (err,rows,fields){
      ...
      for (var i = 0 ; i < rows.length; i++){
        data+="<hr/>";
        data+="<center>";
        data+="<p>date: "+rows[i].NGAYGIAODICH+" </p>";

        if (rows[i].TKNGUOIGUI == accountNumber){
          data+="<p>send to "+rows[i].TKNGUOINHAN + " "+ rows[i].SOTIEN+" vnd</p>";
          data+="<p>your balance was down to "+rows[i].SODUNGUOIGUI+" vnd</p>";
        }
        else {
          data+="<p>receive from "+rows[i].TKNGUOIGUI + " "+ rows[i].SOTIEN+" vnd</p>"; 
          data+="<p>your balance was up to "+rows[i].SODUNGUOINHAN+" vnd</p>";
        }
